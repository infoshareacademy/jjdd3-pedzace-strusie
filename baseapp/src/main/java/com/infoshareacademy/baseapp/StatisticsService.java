...
public class StatisticsService {
    ...
    private void printAmountByCategories(List<Expense> expenses, List<Income> incomes) {
        ...
        printResults(expenses, incomes);
    }

    private void printResults(List<Expense> expenses, List<Income> incomes) {
    ...
    private void printPeriod(List<Expense> periodList) {
        if (periodList.size() == 0) {
            System.out.println("No expenses for this period!");
        } else {
    ...
    public void printAmountByDateByPeriod(List<Expense> expenses, List<Income> incomes, LocalDate minDatePeriod, LocalDate maxDatePeriod) {
        List<Expense> periodExpensesList = getPeriodExpenses(expenses, minDatePeriod, maxDatePeriod);

        List<Income> periodIncomesList = getPeriodIncomes(incomes, minDatePeriod, maxDatePeriod);

        printAmountByDate(periodExpensesList, periodIncomesList);
    }

    private void printAmountByDate(List<Expense> expenses, List<Income> incomes) {
        printPeriod(expenses);
    ...
    public void printAmountByDateByCategoriesByPeriod(List<Expense> expenses, List<Income> incomes, LocalDate minDatePeriod, LocalDate maxDatePeriod) {
        List<Expense> periodExpensesList = getPeriodExpenses(expenses, minDatePeriod, maxDatePeriod);

        List<Income> periodIncomesList = getPeriodIncomes(incomes, minDatePeriod, maxDatePeriod);

        printAmountByDateByCategories(periodExpensesList, periodIncomesList);
    }

    private void printAmountByDateByCategories(List<Expense> expenses, List<Income> incomes) {
        printPeriod(expenses);
